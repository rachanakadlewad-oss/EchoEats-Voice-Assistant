...
export const GlobalStateProvider = ({ children }) => {
  ...
  const updateQuantity = useCallback(async (foodId, delta) => {
    ...
    try {
      ...
      if (isLoggedIn && user) {
        response = await fetch(`${API_URL}/update-quantity/${foodId}/`, {
          method: 'POST',
        ...
        response = await fetch(`${API_URL}/session-cart/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    try {
      if (isLoggedIn && user) {
        ...
        for (const item of itemsInCart) {
          await fetch(`${API_URL}/update-quantity/${item.FoodID}/`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        await fetch(`${API_URL}/session-cart/clear/${sessionId}/`, {
          method: 'DELETE',
        })
      }
  ...
  const transferSessionCartToUser = useCallback(async () => {
    try {
      const sessionRes = await fetch(`${API_URL}/session-cart/${sessionId}/`)
      const sessionCart = await sessionRes.json()

      for (const item of sessionCart) {
        await fetch(`${API_URL}/update-quantity/${item.food_id}/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const logout = useCallback(async () => {
    ...
    try {
      await fetch(`${API_URL}/logout/${user.user_id}/`, { method: 'POST' })
    } catch (error) {
